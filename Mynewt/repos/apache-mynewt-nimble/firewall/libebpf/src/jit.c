...
__ramfunc static uint8_t jit_buffer[4096];
__ramfunc static uint8_t offset_mem[1024];
#else
static uint8_t jit_buffer[4096];
...
void gen_jit_code(struct ebpf_vm *vm)
{
    ...
    state.err_line = 0;
    state.__bpf_call_base = (void *)(uint32_t)vm->helper_func;
