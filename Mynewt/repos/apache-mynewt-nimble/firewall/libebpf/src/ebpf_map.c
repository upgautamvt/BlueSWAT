...
typedef struct Hashmap
{
    DArray *buckets;
    u32 bucket_num;
    _Bool (*Hashmap_cmp)(const void *, const void *);
    u32 (*Hashmap_hash)(const void *key);
} Hashmap;
...
static inline int Hashmap_get_node(Hashmap *map, u32 hash, DArray *bucket, void *key)
{
    ...
    {
        DArray *node_array = DArray_get(bucket, i);
        HashmapNode *node = (HashmapNode *)node_array;
...
void *Hashmap_delete(Hashmap *map, void *key)
{
    ...
    DArray *cur_array = DArray_get(bucket, i);
    HashmapNode *cur = (HashmapNode *)cur_array;
