...
static bool bounds_check(const struct ebpf_vm *vm, void *addr, int size,
                         const char *type, u16 cur_pc, void *mem,
...
{
    struct ebpf_vm *vm =
        (struct ebpf_vm *)ebpf_calloc(1, sizeof(struct ebpf_vm));

    // printf("Create eBPF vm size: %d\n", sizeof(struct ebpf_vm));
...
void ebpf_vm_set_inst(struct ebpf_vm *vm, const uint8_t *code,
                      uint32_t code_len)
{
    vm->insts = (struct ebpf_inst *)code;
    vm->num_insts = (u16)code_len / sizeof(vm->insts[0]);
                  ...
                  void *fn)
{
    if (vm->helper_func == NULL)
    {
        ...
        {
            g_helepr_func =
                ebpf_calloc(1, sizeof(ebpf_helper_func));
            g_helepr_func->ext_funcs =
                ebpf_calloc(MAX_EXT_FUNCS, sizeof(ext_func));
        }
    ...
    // DEBUG_LOG("register helper func 0x%08x 0x%08x 0x%08x\n", g_helepr_func, fn, vm->helper_func->ext_funcs[idx]);
...
#define BOUNDS_CHECK_STORE(size)                                   \
    ...
    while (true)
    {
        ...
        case EBPF_OP_MOV64_IMM:
            DST = IMM;
            // DEBUG_LOG("MOV64_IMM r[%d]=%d pc=%d\n", inst->dst, inst->imm, cur_pc);
            break;
        ...
        case EBPF_OP_JSLE_REG:
            ...
            break;
        case EBPF_OP_CALL:
            // DEBUG_LOG("args: r1=%d, r2=%d, r3=%d pc=%d\n", (s32) reg[1], (s32) reg[2], (s32) reg[3], cur_pc);
            reg[0] = vm->helper_func->ext_funcs[inst->imm](
                (u32)reg[1], (u32)reg[2], (u32)reg[3],
                (u32)reg[4], (u32)reg[5]);
            break;
        case EBPF_OP_EXIT:
            return reg[0];
...
bool bounds_check(const struct ebpf_vm *vm, void *addr, int size,
                  const char *type, u16 cur_pc, void *mem, size_t mem_len,
                  void *stack)
{
    ...
    if (mem &&
        (addr >= mem && ((u64 *)addr + size) <= ((u64 *)mem + mem_len)))
    {
        return true;
    }
    else if (addr >= stack &&
             ((u64 *)addr + size) <= ((u64 *)stack + STACK_SIZE))
    {
        return true;
    }
    else
