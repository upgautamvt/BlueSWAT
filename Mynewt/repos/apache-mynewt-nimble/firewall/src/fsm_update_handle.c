...
struct policy_cache_new *policy_arr_new;
...
bool ifw_add_policy(struct fsm_policy policy, void *dst)
{
    ...
    policy_arr_new = (struct policy_cache_new *)my_os_realloc(policy_arr_new, new_policy_count * sizeof(struct policy_cache_new));
...
bool ifw_remove_policy(struct fsm_policy policy)
{
    int pid = 0;

    pid = PID_##policy.index;
    remove_policy(policy.class, policy.type, pid);
