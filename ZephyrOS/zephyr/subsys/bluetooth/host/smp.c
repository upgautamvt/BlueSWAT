...
#define BT_SMP_AUTH_DEFAULT                                                    \
	(BT_SMP_AUTH_BONDING_FLAGS | BT_SMP_AUTH_SC | BT_SMP_AUTH_CT2)
#else
...
#define DISPLAY_FIXED(smp)                                                     \
	(IS_ENABLED(CONFIG_BT_FIXED_PASSKEY) &&                                \
...
static const u8_t sc_debug_public_key[64] = {
	...
	0x2c, 0x5e, 0x2c, 0xbe, 0x97, 0xf2, 0xd2, 0x03, 0xb0, 0x20, 0x8b,
	0xd2, 0x89, 0x15, 0xd0, 0x8e, 0x1c, 0x74, 0x24, 0x30, 0xed, 0x8f,
	0xc2, 0x45, 0x63, 0x76, 0x5c, 0x15, 0x52, 0x5a, 0xbf, 0x9a, 0x32,
	0x63, 0x6d, 0xeb, 0x2a, 0x65, 0x49, 0x9c, 0x80, 0xdc
};
...
static bool smp_keys_check(struct bt_conn *conn)
{
	...
	if (!conn->le.keys) {
		conn->le.keys =
			bt_keys_find(BT_KEYS_LTK_P256, conn->id, &conn->le.dst);
		if (!conn->le.keys) {
...
static int smp_f4(const u8_t *u, const u8_t *v, const u8_t *x, u8_t z,
		  u8_t res[16])
{
	u8_t xs[16];
	u8_t m[65];
...
static int smp_f6(const u8_t *w, const u8_t *n1, const u8_t *n2, const u8_t *r,
		  const u8_t *iocap, const bt_addr_le_t *a1,
...
static int smp_g2(const u8_t u[32], const u8_t v[32], const u8_t x[16],
		  const u8_t y[16], u32_t *passkey)
{
	u8_t m[80], xs[16];
	int err;
...
static u8_t smp_br_ident_addr_info(struct bt_smp_br *smp, struct net_buf *buf)
{
	struct bt_conn *conn = smp->chan.chan.conn;
...
static int smp_c1(const u8_t k[16], const u8_t r[16], const u8_t preq[7],
		  const u8_t pres[7], const bt_addr_le_t *ia,
		  const bt_addr_le_t *ra, u8_t enc_data[16])
{
	u8_t p1[16], p2[16];
	int err;
...
#if !defined(CONFIG_BT_SMP_SC_PAIR_ONLY)
static int smp_s1(const u8_t k[16], const u8_t r1[16], const u8_t r2[16],
		  u8_t out[16])
{
...
static u8_t legacy_pairing_req(struct bt_smp *smp)
{
	...
	if ((DISPLAY_FIXED(smp) || smp->method == JUST_WORKS) &&
	    !atomic_test_bit(smp->flags, SMP_FLAG_SEC_REQ) && bt_auth &&
	    bt_auth->pairing_confirm) {
		atomic_set_bit(smp->flags, SMP_FLAG_USER);
...
bool bt_smp_request_ltk(struct bt_conn *conn, u64_t rand, u16_t ediv, u8_t *ltk)
{
	...
	if (ediv == 0U && rand == 0U && conn->le.keys &&
	    (conn->le.keys->keys & BT_KEYS_LTK_P256)) {
...
static int smp_send_security_req(struct bt_conn *conn)
{
	...
	req_buf =
		smp_create_pdu(smp, BT_SMP_CMD_SECURITY_REQUEST, sizeof(*req));
	if (!req_buf) {
		return -ENOBUFS;
	}
...
static u8_t smp_pairing_req(struct bt_smp *smp, struct net_buf *buf)
{
	...
	rsp->oob_flag =
		oobd_present ? BT_SMP_OOB_PRESENT : BT_SMP_OOB_NOT_PRESENT;
...
static u8_t smp_security_request(struct bt_smp *smp, struct net_buf *buf)
{
	...
	if (bt_conn_le_start_encryption(
...
static void bt_smp_encrypt_change(struct bt_l2cap_chan *chan, u8_t hci_status)
{
	struct bt_smp *smp = CONTAINER_OF(chan, struct bt_smp, chan);
 ...
 */
static const u8_t key[] = { 0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6,
			    0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c };

static const u8_t M[] = { 0x6b, 0xc1, 0xbe, 0xe2, 0x2e, 0x40, 0x9f, 0x96,
...
{
	u8_t mac1[] = { 0xbb, 0x1d, 0x69, 0x29, 0xe9, 0x59, 0x37, 0x28,
			0x7f, 0xa3, 0x7d, 0x12, 0x9b, 0x75, 0x67, 0x46 };
	u8_t mac2[] = { 0x07, 0x0a, 0x16, 0xb4, 0x6b, 0x4d, 0x41, 0x44,
			0xf7, 0x9b, 0xdd, 0x9d, 0xd0, 0x4a, 0x28, 0x7c };
	u8_t mac3[] = { 0xdf, 0xa6, 0x67, 0x47, 0xde, 0x9a, 0xe6, 0x30,
			0x30, 0xca, 0x32, 0x61, 0x14, 0x97, 0xc8, 0x27 };
	u8_t mac4[] = { 0x51, 0xf0, 0xbe, 0xbf, 0x7e, 0x3b, 0x9d, 0x92,
...
{
	const u8_t sig1[] = { 0x00, 0x00, 0x00, 0x00, 0xb3, 0xa8,
			      0x59, 0x41, 0x27, 0xeb, 0xc2, 0xc0 };
	const u8_t sig2[] = { 0x00, 0x00, 0x00, 0x00, 0x27, 0x39,
			      0x74, 0xf4, 0x39, 0x2a, 0x23, 0x2a };
	const u8_t sig3[] = { 0x00, 0x00, 0x00, 0x00, 0xb7, 0xca,
			      0x94, 0xab, 0x87, 0xc7, 0x82, 0x18 };
	const u8_t sig4[] = { 0x00, 0x00, 0x00, 0x00, 0x44, 0xe1,
...
static int smp_f5_test(void)
{
	...
	u8_t exp_ltk[16] = { 0x38, 0x0a, 0x75, 0x94, 0xb5, 0x22, 0x05, 0x98,
			     0x23, 0xcd, 0xd7, 0x69, 0x11, 0x79, 0x86, 0x69 };
	u8_t exp_mackey[16] = { 0x20, 0x6e, 0x63, 0xce, 0x20, 0x6a, 0x3f, 0xfd,
