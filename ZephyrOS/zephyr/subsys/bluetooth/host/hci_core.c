...
NET_BUF_POOL_FIXED_DEFINE(hci_cmd_pool, CONFIG_BT_HCI_CMD_COUNT, CMD_BUF_SIZE,
			  NULL);
				...
				size_t num_handlers)
{
	...
	for (i = 0; i < num_handlers; i++) {
		...
		if (buf->len < handler->min_len) {
			BT_ERR("Too small (%u bytes) event 0x%02x", buf->len,
			       event);
			return;
		}
...
static int set_random_address(const bt_addr_t *addr)
{
	...
	BT_DBG("set_random_address start: %s", bt_addr_str(addr));
	...
	//	printk("set_random_address end: %s\n", bt_addr_str(bt_dev.random_addr));
	return 0;
}
...
#endif
	if (IS_ENABLED(CONFIG_BT_CENTRAL) && IS_ENABLED(CONFIG_BT_WHITELIST) &&
...
static void rpa_timeout(struct k_work *work)
{
	...
	if (IS_ENABLED(CONFIG_BT_CENTRAL)) {
		struct bt_conn *conn = bt_conn_lookup_state_le(
			BT_ID_DEFAULT, NULL, BT_CONN_CONNECT_SCAN);
...
static int set_le_scan_enable(u8_t enable)
{
	...
	if (enable == BT_HCI_LE_SCAN_ENABLE) {
		cp->filter_dup =
			atomic_test_bit(bt_dev.flags, BT_DEV_SCAN_FILTER_DUP);
	} else {
...
static void le_phy_update_complete(struct net_buf *buf)
{
	...
	BT_DBG("PHY updated: status: 0x%02x, tx: %u, rx: %u", evt->status,
...
static bool br_sufficient_key_size(struct bt_conn *conn)
{
	...
	err = bt_hci_cmd_send_sync(BT_HCI_OP_READ_ENCRYPTION_KEY_SIZE, buf,
				   &rsp);
	if (err) {
...
static int addr_res_enable(u8_t enable)
{
	...
	return bt_hci_cmd_send_sync(BT_HCI_OP_LE_SET_ADDR_RES_ENABLE, buf,
				    NULL);
}
...
static void le_adv_report(struct net_buf *buf)
{
	...
	while (num_reports--) {
		...
		BT_DBG("%s event %u, len %u, rssi %d dBm",
		       bt_addr_le_str(&info->addr), info->evt_type,
		       info->length, rssi);
...
static const struct event_handler normal_events[] = {
	...
	EVENT_HANDLER(BT_HCI_EVT_REMOTE_FEATURES, read_remote_features_complete,
...
static void send_cmd(void)
{
	...
	BT_DBG("Sending command 0x%04x (buf %p) to driver", cmd(buf)->opcode,
	       buf);
...
{
	const char *const str[] = { "1.0b", "1.1", "1.2", "2.0", "2.1", "3.0",
...
{
	static const char *const nordic_str[] = { "reserved", "nRF51x",
						  "nRF52x", "nRF53x" };
...
static void hci_vs_init(void)
{
	...
	err = bt_hci_cmd_send_sync(BT_HCI_OP_VS_READ_SUPPORTED_COMMANDS, NULL,
				   &rsp);
	if (err) {
	...
	err = bt_hci_cmd_send_sync(BT_HCI_OP_VS_READ_SUPPORTED_FEATURES, NULL,
				   &rsp);
	if (err) {
...
int bt_recv(struct net_buf *buf)
{
	...
	case BT_BUF_ACL_IN:
		...
		// IFW below
		// IFW_DEBUG_LOG("Invoke hci_acl here.");
		// IFW above
...
int bt_enable(bt_ready_cb_t cb)
{
	...
	if (IS_ENABLED(CONFIG_BT_SETTINGS)) {
		...
		// CONFIG_BT_DEVICE_NAME is in prj.conf
	...
	k_thread_create(&tx_thread_data, tx_thread_stack,
			K_THREAD_STACK_SIZEOF(tx_thread_stack), hci_tx_thread,
			NULL, NULL, NULL, K_PRIO_COOP(CONFIG_BT_HCI_TX_PRIO), 0,
			K_NO_WAIT);
...
static uint8_t bt_read_static_addr(struct bt_hci_vs_static_addr *addrs)
{
	...
	if (IS_ENABLED(CONFIG_BT_HCI_VS_EXT_DETECT) &&
	    rsp->len !=
		    (sizeof(struct bt_hci_rp_vs_read_static_addrs) +
...
int bt_setup_random_id_addr(void)
{
	...
	printk("bt_setup_random_id_addr----------->\n");
			     ...
			     const bt_addr_le_t *peer)
{
	...
	// IFW below
	// IFW_DEBUG_LOG("Invoke bt_hci_cmd_create here.");
	// IFW above
	...
	// IFW below
	// IFW_DEBUG_LOG("Invoke bt_hci_cmd_send_sync here.");
	// IFW above
	...
	if (!dir_adv) {
		// IFW below
		// IFW_DEBUG_LOG("Invoke le_adv_update here.");
		// IFW above
		...
		if (IS_ENABLED(CONFIG_BT_PERIPHERAL) &&
			...
			// IFW below
			// IFW_DEBUG_LOG(
				// "Invoke bt_conn_set_state to BT_CONN_CONNECT_ADV here");
				// IFW above
...
int bt_le_adv_stop(void)
{
	...
	if (IS_ENABLED(CONFIG_BT_PERIPHERAL)) {
		...
		// IFW below
		// IFW_DEBUG_LOG(
			// "Invoke bt_conn_set_state to BT_CONN_DISCONNECTED here");
			// IFW above
...
void bt_le_adv_resume(void)
{
	...
	// IFW_DEBUG_LOG
	// IFW_DEBUG_LOG("Invoke bt_conn_set_state to BT_CONN_CONNECT_ADV here");
	// IFW above
...
int bt_le_set_chan_map(u8_t chan_map[5])
{
	...
	return bt_hci_cmd_send_sync(BT_HCI_OP_LE_SET_HOST_CHAN_CLASSIF, buf,
				    NULL);
}
...
int bt_br_discovery_stop(void)
{
	...
	for (i = 0; i < discovery_results_count; i++) {
			...
			priv = (struct discovery_priv *)&discovery_results[i]
				       ._priv;
			...
			bt_hci_cmd_send_sync(BT_HCI_OP_REMOTE_NAME_CANCEL, buf,
					     NULL);
	}
