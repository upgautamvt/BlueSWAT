...
static struct bt_l2cap_le_chan *__l2cap_lookup_ident(struct bt_conn *conn,
						     u16_t ident, bool remove)
{
	struct bt_l2cap_chan *chan;
...
static struct net_buf *l2cap_create_le_sig_pdu(struct net_buf *buf, u8_t code,
					       u8_t ident, u16_t len)
{
...
{
	return bt_conn_create_pdu_timeout(
		pool, sizeof(struct bt_l2cap_hdr) + reserve, timeout);
}
...
static void l2cap_send_reject(struct bt_conn *conn, u8_t ident, u16_t reason,
			      void *data, u8_t data_len)
{
...
int bt_l2cap_server_register(struct bt_l2cap_server *server)
{
	...
	if (server->psm) {
		...
		for (psm = L2CAP_LE_PSM_DYN_START; psm <= L2CAP_LE_PSM_DYN_END;
		     psm++) {
	...
	if (!chan->rx.init_credits) {
		if (chan->chan.ops->alloc_buf) {
			...
			chan->rx.init_credits =
				(chan->rx.mtu + (L2CAP_MAX_LE_MPS - 1)) /
				L2CAP_MAX_LE_MPS;
		} else {
...
static void le_conn_rsp(struct bt_l2cap *l2cap, u8_t ident, struct net_buf *buf)
{
	...
	BT_DBG("dcid 0x%04x mtu %u mps %u credits %u result 0x%04x", dcid, mtu,
	       mps, credits, result);
			      ...
			      u16_t sdu_hdr_len)
{
	...
	BT_DBG("ch %p cid 0x%04x len %u credits %u", ch, ch->tx.cid, seg->len,
...
{
	/// Note: Here parse the l2cap SIGNALING channel packets
...
{
	// IFW_DEBUG_LOG("Invoke bt_l2cap_chan_recv_complete here.");
...
static void l2cap_connected(struct bt_l2cap_chan *chan)
{
	...
	// IFW_DEBUG_LOG("Invoke l2cap_connected here.");
}
...
static void l2cap_disconnected(struct bt_l2cap_chan *chan)
{
	...
	// IFW_DEBUG_LOG("Invoke l2cap_disconnected here.");
}

static int l2cap_accept(struct bt_conn *conn, struct bt_l2cap_chan **chan)
{
	// IFW_DEBUG_LOG("Invoke l2cap_accept here.");
...
static int l2cap_le_connect(struct bt_conn *conn, struct bt_l2cap_le_chan *ch,
			    u16_t psm)
{
	// IFW_DEBUG_LOG("Invoke l2cap_le_connect here.");
