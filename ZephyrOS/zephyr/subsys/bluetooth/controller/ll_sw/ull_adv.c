...
u8_t ll_adv_scan_rsp_set(u8_t len, u8_t const *const data)
{
	...
	// IFW_FSM_CHECK_UPDATE(len, SCAN_RSP_LEN, CONN);
	...
	// IFW below
	// IFW_DEBUG_LOG("lll_adv_scan_rsp_enqueue here.");
	// IFW above
...
u8_t ll_adv_enable(u8_t enable)
{
	...
	if ((pdu_adv->type == PDU_ADV_TYPE_ADV_IND) ||
		...
		// IFW below
		// IFW_DEBUG_LOG("prepare connectable advertising");
		// IFW above
...
#endif
	{
		...
		const u16_t conn_ind_us =
			ll_hdr_us +
			BYTES2US(INITA_SIZE + ADVA_SIZE + LLDATA_SIZE, phy);
		const u8_t scan_req_us =
			ll_hdr_us + BYTES2US(SCANA_SIZE + ADVA_SIZE, phy);
		/* ll_header plus AdvA and scan response data */
		const u16_t scan_rsp_us =
			ll_hdr_us + BYTES2US(ADVA_SIZE + rsp_data_len, phy);
