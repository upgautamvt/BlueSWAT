...
static void event_inactive(u32_t ticks_at_expire, u32_t remainder, u16_t lazy,
			   void *context);
...
u32_t radio_init(void *hf_clock, u8_t sca, void *entropy,
		 u8_t connection_count_max, u8_t rx_count_max,
		 u8_t tx_count_max, u16_t packet_data_octets_max,
		 u16_t packet_tx_data_size, u8_t *mem_radio, u16_t mem_size)
{
	// IFW below
	// IFW_DEBUG_LOGLOG();
	// IFW above
	...
	_radio.packet_tx_data_size =
		MROUND(offsetof(struct radio_pdu_node_tx, pdu_data) +
		       offsetof(struct pdu_data, lldata) + packet_tx_data_size);
...
static void common_init(void)
{
	...
	if (_radio.connection_count) {
		mem_init(_radio.conn_pool, CONNECTION_T_SIZE,
			 _radio.connection_count, &_radio.conn_free);
	} else {
		_radio.conn_free = NULL;
	}

	/* initialise rx pool. */
	mem_init(_radio.pkt_rx_data_pool, _radio.packet_rx_data_size,
		 _radio.packet_rx_data_count, &_radio.pkt_rx_data_free);
...
static inline void isr_radio_state_tx(void)
{
	...
	case ROLE_SLAVE:
		...
		rx_packet_set(_radio.conn_curr,
			      (void *)_radio.packet_rx[_radio.packet_rx_last]
				      ->pdu_data);
		...
		radio_gpio_pa_lna_enable(
			radio_tmr_tifs_base_get() + RADIO_TIFS - 4 -
...
static inline bool isr_adv_ci_tgta_check(struct pdu_adv *adv,
					 struct pdu_adv *ci, u8_t rl_idx)
{
#if defined(CONFIG_BT_CTLR_PRIVACY)
...
{
	// IFW below
	// IFW_DEBUG_LOG("Peripheral invoked here.");
	// IFW above
	...
	u8_t rl_idx =
		irkmatch_ok ? ctrl_rl_irk_idx(irkmatch_id) : FILTER_IDX_NONE;
#else
	u8_t rl_idx = FILTER_IDX_NONE;
#endif /* CONFIG_BT_CTLR_PRIVACY */

	pdu_adv = (void *)radio_pkt_scratch_get();
	_pdu_adv = (void *)&_radio.advertiser.adv_data
			   .data[_radio.advertiser.adv_data.first][0];
	...
	if ((pdu_adv->type == PDU_ADV_TYPE_SCAN_REQ) &&
		...
		conn->handle = mem_index_get(conn, _radio.conn_pool,
					     CONNECTION_T_SIZE);
		memcpy(&conn->crc_init[0], &pdu_adv->connect_ind.crc_init[0],
		       3);
		memcpy(&conn->access_addr[0],
		       &pdu_adv->connect_ind.access_addr[0], 4);
		memcpy(&conn->data_chan_map[0],
		       &pdu_adv->connect_ind.chan_map[0],
		       sizeof(conn->data_chan_map));
		conn->data_chan_count = util_ones_count_get(
			&conn->data_chan_map[0], sizeof(conn->data_chan_map));
		...
		conn->supervision_reload = RADIO_CONN_EVENTS(
			(pdu_adv->connect_ind.timeout * 10 * 1000),
			conn_interval_us);
		...
		if (irkmatch_ok && rl_idx != FILTER_IDX_NONE) {
			...
			memcpy(&radio_le_conn_cmplt->peer_rpa[0],
			       &pdu_adv->connect_ind.init_addr[0], BDADDR_SIZE);
		} else {
		...
		radio_le_conn_cmplt->interval = pdu_adv->connect_ind.interval;
		radio_le_conn_cmplt->latency = pdu_adv->connect_ind.latency;
		radio_le_conn_cmplt->timeout = pdu_adv->connect_ind.timeout;
		radio_le_conn_cmplt->mca = pdu_adv->connect_ind.sca;
		...
		conn->hdr.ticks_slot = HAL_TICKER_US_TO_TICKS(
			RADIO_TICKER_START_PART_US + rx_ready_delay + 328 +
			RADIO_TIFS + 328);
		...
		ticks_slot_offset = MAX(conn->hdr.ticks_active_to_start,
					conn->hdr.ticks_xtal_to_start);
		conn_interval_us -= conn->slave.window_widening_periodic_us;

		conn_offset_us = radio_tmr_end_get();
		conn_offset_us +=
			((u64_t)pdu_adv->connect_ind.win_offset + 1) * 1250;
		 ...
		 */
		u32_t mayfly_was_enabled = mayfly_is_enabled(
			RADIO_TICKER_USER_ID_WORKER, RADIO_TICKER_USER_ID_JOB);
		...
		ticker_status =
			ticker_stop(RADIO_TICKER_INSTANCE_ID_RADIO,
				    ...
				    RADIO_TICKER_ID_ADV, ticker_stop_adv_assert,
				    (void *)__LINE__);
...
static u32_t isr_rx_scan_report(u8_t rssi_ready, u8_t rl_idx, bool dir_report)
{
	...
	if (0) {
#if defined(CONFIG_BT_HCI_MESH_EXT)
	} else if (_radio.advertiser.is_enabled && _radio.advertiser.is_mesh) {
				     ...
				     u8_t rl_idx)
{
	...
	return (((_radio.scanner.filter_policy & 0x01) == 0) &&
		...
		(devmatch_ok || ctrl_irk_whitelisted(rl_idx)));
#else
	return ((_radio.scanner.filter_policy & 0x01) == 0) || devmatch_ok;
#endif /* CONFIG_BT_CTLR_PRIVACY */
}
...
static inline bool isr_scan_rsp_adva_matches(struct pdu_adv *srsp)
{
	...
	return ((sreq->rx_addr == srsp->tx_addr) &&
		(memcmp(&sreq->scan_req.adv_addr[0], &srsp->scan_rsp.addr[0],
			BDADDR_SIZE) == 0));
}

static inline bool isr_scan_init_adva_check(struct pdu_adv *pdu, u8_t rl_idx)
{
#if defined(CONFIG_BT_CTLR_PRIVACY)
	...
	return ((_radio.scanner.adv_addr_type == pdu->tx_addr) &&
		(memcmp(&_radio.scanner.adv_addr[0], &pdu->adv_ind.addr[0],
			BDADDR_SIZE) == 0));
}
...
{
#if defined(CONFIG_BT_CTLR_PRIVACY)
	if (ctrl_rl_addr_resolve(pdu->rx_addr, pdu->direct_ind.tgt_addr,
				 rl_idx)) {
		return true;
	...
	return (((_radio.scanner.init_addr_type == pdu->rx_addr) &&
		 (memcmp(&_radio.scanner.init_addr[0],
			 &pdu->direct_ind.tgt_addr[0], BDADDR_SIZE) == 0))) ||
				...
				u8_t rssi_ready)
{
	...
	if (0) {
...
#endif /* CONFIG_BT_CTLR_PRIVACY */
				pdu_adv_tx->tx_addr =
					_radio.scanner.init_addr_type;
				memcpy(&pdu_adv_tx->connect_ind.init_addr[0],
				       &_radio.scanner.init_addr[0],
				       BDADDR_SIZE);
			}
		 ...
		 */
			...
			pdu_adv_tx->connect_ind.hop = conn->data_chan_hop;
			pdu_adv_tx->connect_ind.sca = _radio.sca;

			radio_switch_complete_and_disable();

			// PDU_ADV_TYPE_CONNECT_IND in pdu_adv_tx sent here
			// STATE_TRANS: initiating ---> connection / initiator ---> master
			radio_pkt_tx_set(pdu_adv_tx);

#if defined(CONFIG_BT_CTLR_GPIO_PA_PIN)
			radio_gpio_pa_setup();
			radio_gpio_pa_lna_enable(
				radio_tmr_tifs_base_get() + RADIO_TIFS -
				radio_rx_chain_delay_get(0, 0) -
				CONFIG_BT_CTLR_GPIO_PA_OFFSET);
#endif /* CONFIG_BT_CTLR_GPIO_PA_PIN */
		 ...
		 */
...
#if defined(CONFIG_BT_CTLR_PRIVACY)
			radio_le_conn_cmplt->own_addr_type =
				pdu_adv_tx->tx_addr;
			memcpy(&radio_le_conn_cmplt->own_addr[0],
			...
			radio_le_conn_cmplt->timeout =
				_radio.scanner.conn_timeout;
			radio_le_conn_cmplt->mca = pdu_adv_tx->connect_ind.sca;
			...
			if (IS_ENABLED(CONFIG_BT_CTLR_CHAN_SEL_2)) {
				...
				if (pdu_adv_rx_chan_sel) {
					u16_t aa_ls =
						((u16_t)conn->access_addr[1]
						 << 8) |
						conn->access_addr[0];
					u16_t aa_ms =
						((u16_t)conn->access_addr[3]
						 << 8) |
						conn->access_addr[2];
		 ...
		 */
			...
			ticker_status =
				ticker_stop(RADIO_TICKER_INSTANCE_ID_RADIO,
					    ...
					    (void *)__LINE__);
			ticker_stop_scan_assert(ticker_status,
						(void *)__LINE__);
		...
		/* Active scanner */
			...
			/* save the adv packet */
			err = isr_rx_scan_report(
				rssi_ready,
				irkmatch_ok ? rl_idx : FILTER_IDX_NONE, false);
			if (err) {
				return err;
			}
		...
		/* Passive scanner or scan responses */
			  ...
			  ((pdu_adv_rx->type == PDU_ADV_TYPE_DIRECT_IND) &&
			   (pdu_adv_rx->len ==
			    sizeof(struct pdu_adv_direct_ind)) &&
			   (/* allow directed adv packets addressed to this device */
...
static inline u32_t feat_get(u8_t *features)
{
	...
	feat = ~LL_FEAT_BIT_MASK_VALID | features[0] | (features[1] << 8) |
	       (features[2] << 16);
				  ...
				  u8_t *rx_enqueue)
{
	...
	if (!conn->role &&
	    (rej_ext_ind->error_code == BT_HCI_ERR_UNSUPP_REMOTE_FEATURE)) {
	...
	cp->timeout =
		conn->supervision_reload * conn->conn_interval * 125 / 1000;
...
static inline void isr_rx_conn_pkt_ctrl_rej(struct radio_pdu_node_rx *node_rx,
					    u8_t *rx_enqueue)
{
	struct pdu_data_llctrl_reject_ext_ind *rej_ext_ind;
					    ...
					    u8_t *rx_enqueue)
{
	...
	if (/* Local idle, and Peer request then complete the Peer procedure
	    ...
	    ((_radio.conn_curr->llcp_length.req ==
	      _radio.conn_curr->llcp_length.ack) &&
	     (pdu_data_rx->llctrl.opcode == PDU_DATA_LLCTRL_TYPE_LENGTH_REQ)) ||
	    /* or Local has active... */
	    ((_radio.conn_curr->llcp_length.req !=
	      _radio.conn_curr->llcp_length.ack) &&
	      ...
	      */
	     ((((_radio.conn_curr->llcp_length.state == LLCP_LENGTH_STATE_REQ) ||
		(_radio.conn_curr->llcp_length.state ==
		...
		if (lr->max_tx_octets >= PDU_DC_PAYLOAD_SIZE_MIN) {
			eff_rx_octets =
				MIN(lr->max_tx_octets, LL_LENGTH_OCTETS_RX_MAX);
		}
...
static inline u8_t isr_rx_conn_pkt_ctrl(struct radio_pdu_node_rx *node_rx,
					u8_t *rx_enqueue)
{
	u8_t nack = 0U;
	struct pdu_data *pdu_data_rx;
	...
	case PDU_DATA_LLCTRL_TYPE_CONN_PARAM_REQ:
		...
		if (!_radio.conn_curr->role) {
			...
			if ((conn->llcp_conn_param.req !=
				...
				struct pdu_data_llctrl_conn_param_req *cpr =
					(void *)&pdu_data_rx->llctrl
						.conn_param_req;
				...
				if ((cpr->interval_min < 6) ||
				    ...
				    (cpr->timeout > 3200) ||
				    ((cpr->timeout * 4) <=
				     ((cpr->latency + 1) * cpr->interval_max)) ||
			...
			struct connection *conn = _radio.conn_curr;
			struct pdu_data_llctrl_conn_param_req *cpr =
				(void *)&pdu_data_rx->llctrl.conn_param_req;

			/* Invalid parameters */
			if ((cpr->interval_min < 6) ||
			    (cpr->interval_max > 3200) ||
			    (cpr->interval_min > cpr->interval_max) ||
			    (cpr->latency > 499) || (cpr->timeout < 10) ||
			    (cpr->timeout > 3200) ||
			    ((cpr->timeout * 4) <=
			     ((cpr->latency + 1) * cpr->interval_max)) ||
		...
		if (_radio.role == ROLE_MASTER) {
			if ((_radio.conn_curr->llcp_phy.ack !=
				...
				struct pdu_data_llctrl *c =
					&pdu_data_rx->llctrl;
				struct pdu_data_llctrl_phy_req *p = &c->phy_req;
...
static inline u32_t isr_rx_conn_pkt(struct radio_pdu_node_rx *node_rx,
	 ...
	 */
		...
		(_radio.fc_handle[TRIPLE_BUFFER_SIZE - 1] ==
		 _radio.conn_curr->handle)) ||
	       ((_radio.fc_req != 0) && (_radio.fc_handle[_radio.fc_req - 1] ==
					 _radio.conn_curr->handle)))))) {
		...
		if (pdu_data_rx->len != 0) {
			...
			if (_radio.conn_curr->enc_rx) {
				...
				if (mic_failure && _radio.conn_curr->pause_rx &&
				    (pdu_data_rx->ll_id ==
				     PDU_DATA_LLID_CTRL)) {
...
static inline void isr_rx_conn(u8_t crc_ok, u8_t trx_done, u8_t rssi_ready)
{
	struct radio_pdu_node_rx *node_rx;
	...
	radio_gpio_pa_lna_enable(
		radio_tmr_tifs_base_get() + RADIO_TIFS -
		radio_rx_chain_delay_get(_radio.conn_curr->phy_rx, 1) -
		CONFIG_BT_CTLR_GPIO_PA_OFFSET);
#else /* !CONFIG_BT_CTLR_PHY */
				      ...
				      u8_t rssi_ready)
{
	...
	if (!((trx_done) ||
	      ((SILENT_CONNECTION) && (_radio.role == ROLE_SLAVE)))) {
		_radio.state = STATE_CLOSE;
		radio_disable();
...
#if defined(CONFIG_BT_CTLR_PRIVACY)
		rl_idx = devmatch_ok ?
				 ctrl_rl_idx(!!(_radio.scanner.filter_policy &
						0x01),
					     devmatch_id) :
...
static inline u32_t isr_close_adv_mesh(void)
{
	...
	if (!_radio.advertiser.retry) {
		ticker_status =
			ticker_stop(RADIO_TICKER_INSTANCE_ID_RADIO,
				    RADIO_TICKER_USER_ID_WORKER,
				    RADIO_TICKER_ID_ADV, ticker_stop_adv_stop,
				    (void *)__LINE__);
		LL_ASSERT((ticker_status == TICKER_STATUS_SUCCESS) ||
			  (ticker_status == TICKER_STATUS_BUSY));
	...
	_radio.scanner.ticks_window = HAL_TICKER_US_TO_TICKS(
		_radio.advertiser.scan_window_ms * 1000 + radio_tmr_end_get() -
		_radio.mesh_adv_end_us);

	_radio.scanner.hdr.ticks_active_to_start = _radio.ticks_active_to_start;
	...
	ticks_anchor =
		...
		HAL_TICKER_US_TO_TICKS(_radio.mesh_adv_end_us +
				       _radio.advertiser.scan_delay_ms * 1000 -
				       RADIO_TICKER_XTAL_OFFSET_US);

	ticker_status = ticker_start(
		RADIO_TICKER_INSTANCE_ID_RADIO, RADIO_TICKER_USER_ID_WORKER,
		RADIO_TICKER_ID_SCAN, ticks_anchor, 0, 0, 0, TICKER_NULL_LAZY,
		(ticks_slot_offset + _radio.scanner.hdr.ticks_slot),
		event_scan_prepare, NULL, ticker_start_mesh_scan, NULL);
	LL_ASSERT((ticker_status == TICKER_STATUS_SUCCESS) ||
		  (ticker_status == TICKER_STATUS_BUSY));
...
#if defined(CONFIG_BT_HCI_MESH_EXT)
	if (_radio.advertiser.is_mesh && _radio.state == STATE_CLOSE &&
	    !_radio.mesh_adv_end_us) {
	...
	if ((_radio.state == STATE_CLOSE) &&
		...
		if ((_radio.state == STATE_CLOSE) &&
			...
			ticker_status = ticker_update(
				RADIO_TICKER_INSTANCE_ID_RADIO,
				RADIO_TICKER_USER_ID_WORKER,
				RADIO_TICKER_ID_ADV, random_delay, 0, 0, 0, 0,
				0, ticker_update_adv_assert, (void *)__LINE__);
			LL_ASSERT(
				(ticker_status == TICKER_STATUS_SUCCESS) ||
				(ticker_status == TICKER_STATUS_BUSY) ||
...
static inline void isr_close_conn(void)
{
	...
	if (_radio.packet_counter &&
		...
		if (_radio.role == ROLE_SLAVE) {
			...
			start_to_address_expected_us =
				...
				(RADIO_TICKER_JITTER_US << 1) +
				preamble_to_addr_us + window_widening_event_us;
			...
			if (_radio.conn_curr->pkt_tx_head) {
				...
				pdu_data_tx = (void *)_radio.conn_curr
						      ->pkt_tx_head->pdu_data;
				if (pdu_data_tx->len ||
				    _radio.conn_curr->packet_tx_head_offset) {
		...
		if (_radio.conn_curr->supervision_expire > elapsed_event) {
			 ...
			 */
				...
				if (latency_event != 0) {
					...
					force = _radio.conn_curr->slave.force &
						0x01;
...
#endif /* !CONFIG_BT_CTLR_DATA_LENGTH */
#else /* !CONFIG_BT_CTLR_PHY */
		ready_delay = (conn->role) ? radio_rx_ready_delay_get(0, 0) :
					     radio_tx_ready_delay_get(0, 0);
		tx_time = RADIO_PKT_TIME(conn->max_tx_octets, 0);
		if (conn->evt_len_adv) {
			rx_time =
				RADIO_PKT_TIME(conn->llcp_length.rx_octets, 0);
		} else {
	...
	if ((ticks_drift_plus != 0) || (ticks_drift_minus != 0) ||
	    (ticks_slot_plus != 0) || (ticks_slot_minus != 0) || (lazy != 0) ||
	    (force != 0)) {
		u32_t ticker_status;
		...
		ticker_status =
			ticker_update(RADIO_TICKER_INSTANCE_ID_RADIO,
				      ...
				      force, ticker_update_conn_assert,
...
static void event_active(u32_t ticks_at_expire, u32_t remainder, u16_t lazy,
			 void *context)
{
	static memq_link_t s_link;
...
static void event_xtal(u32_t ticks_at_expire, u32_t remainder, u16_t lazy,
		       void *context)
{
	static memq_link_t s_link;
...
static void mayfly_xtal_retain(u8_t caller_id, u8_t retain)
{
	...
	if (retain) {
		if (!s_xtal_retained) {
			static memq_link_t s_link;
			static struct mayfly s_mfy_xtal_start = {
				0, 0, &s_link, NULL, mayfly_xtal_start
			};
			u32_t retval;
			       ...
			       u8_t ticker_id)
{
	if (hdr->ticks_xtal_to_start & XON_BITMASK) {
		u32_t ticker_status;
		u32_t ticks_prepare_to_start = MAX(hdr->ticks_active_to_start,
		...
		ticker_status =
			ticker_update(RADIO_TICKER_INSTANCE_ID_RADIO,
				      ticker_user_id, ticker_id, 0,
				      ticks_drift_minus, ticks_drift_minus, 0,
				      0, 0, prepare_normal, hdr);
		LL_ASSERT((ticker_status == TICKER_STATUS_SUCCESS) ||
			  (ticker_status == TICKER_STATUS_BUSY));
	}
}
...
static void mayfly_xtal_stop_calc(void *params)
{
	...
	if ((ticker_id_next == TICKER_NULL) ||
	    ...
	    (ticker_id_next >=
	     (RADIO_TICKER_ID_FIRST_CONNECTION + _radio.connection_count))) {
	...
	if (ticks_to_expire >
		...
		if (!(hdr_next->ticks_xtal_to_start & XON_BITMASK) &&
			...
			ticker_status =
				ticker_update(RADIO_TICKER_INSTANCE_ID_RADIO,
					      RADIO_TICKER_USER_ID_JOB,
					      ticker_id_next, ticks_drift_plus,
					      0, 0, ticks_drift_plus, 0, 0,
...
static void sched_after_mstr_free_slot_get(u8_t user_id, u32_t ticks_slot_abs,
					   u32_t *ticks_anchor,
					   u32_t *us_offset)
{
					     ...
					     u32_t *win_offset_us)
{
	...
	sched_after_mstr_free_slot_get(
		RADIO_TICKER_USER_ID_JOB,
		(HAL_TICKER_US_TO_TICKS(RADIO_TICKER_XTAL_OFFSET_US) +
		 ticks_slot),
	...
	*win_offset_us += HAL_TICKER_TICKS_TO_US(
		ticker_ticks_diff_get(ticks_anchor_offset, ticks_anchor));
...
static void mayfly_sched_win_offset_use(void *params)
{
	...
	memcpy(conn->llcp.conn_upd.pdu_win_offset, &win_offset, sizeof(u16_t));
}
...
static void sched_free_win_offset_calc(struct connection *conn_curr,
				       ...
				       u16_t conn_interval, u8_t *offset_max,
				       u8_t *win_offset)
{
	...
	if (conn_curr->hdr.ticks_xtal_to_start & XON_BITMASK) {
		...
		ticks_slot_abs =
			conn_curr->hdr.ticks_xtal_to_start & ~XON_BITMASK;
		...
		if ((conn != conn_curr) && (is_select || !conn->role)) {
			...
			if (*ticks_to_offset_next < ticks_to_expire_normal) {
				...
				while ((offset_index < *offset_max) &&
					...
					memcpy(win_offset + (sizeof(u16_t) *
							     offset_index),
	...
	if (ticker_id == 0xFF) {
		...
		while (offset_index < *offset_max) {
			_win_offset =
				HAL_TICKER_TICKS_TO_US(ticks_to_expire_prev +
						       ticks_slot_abs_prev) /
				1250;
			if (_win_offset >= conn_interval) {
				ticks_to_expire_prev = 0U;
...
#define OFFSET_M_MAX 6
	...
	ticks_to_offset =
		HAL_TICKER_US_TO_TICKS(conn->llcp_conn_param.offset0 * 1250);
	...
	if (offset_index_s < OFFSET_S_MAX) {
		...
		/* send reject_ind_ext */
		pdu_ctrl_tx =
			(void *)((u8_t *)conn->llcp.conn_upd.pdu_win_offset -
...
static void mayfly_radio_stop(void *params)
{
	...
	radio_used =
		((_radio.state != STATE_NONE) && (_radio.state != STATE_STOP) &&
		 (_radio.state != STATE_ABORT));
...
static void event_stop(u32_t ticks_at_expire, u32_t remainder, u16_t lazy,
		       void *context)
{
	static memq_link_t s_link;
...
static void event_common_prepare(u32_t ticks_at_expire, u32_t remainder,
				 u32_t *ticks_xtal_to_start,
				 u32_t *ticks_active_to_start,
				 u32_t ticks_preempt_to_start, u8_t ticker_id,
				 ...
				 void *context)
{
	...
	if (_ticks_xtal_to_start & XON_BITMASK) {
		_ticks_xtal_to_start =
			MAX(_ticks_active_to_start, ticks_preempt_to_start);
	}
#endif /* CONFIG_BT_CTLR_XTAL_ADVANCED */
	...
	ticker_status = ticker_start(
		RADIO_TICKER_INSTANCE_ID_RADIO, RADIO_TICKER_USER_ID_WORKER,
		RADIO_TICKER_ID_EVENT, ticks_at_expire, ticks_to_start,
		TICKER_NULL_PERIOD, TICKER_NULL_REMAINDER, TICKER_NULL_LAZY,
		TICKER_NULL_SLOT, ticker_timeout_fp, context,
		ticker_success_assert, (void *)__LINE__);
	LL_ASSERT((ticker_status == TICKER_STATUS_SUCCESS) ||
		  (ticker_status == TICKER_STATUS_BUSY));
...
#define RADIO_DEFERRED_PREEMPT 0
	...
	if (_radio.state != STATE_NONE) {
		ticker_status = ticker_start(
			RADIO_TICKER_INSTANCE_ID_RADIO,
			RADIO_TICKER_USER_ID_WORKER, RADIO_TICKER_ID_PRE_EMPT,
			ticks_at_expire,
			(ticks_to_start - conn->hdr.ticks_preempt_to_start),
			TICKER_NULL_PERIOD, TICKER_NULL_REMAINDER,
			TICKER_NULL_LAZY, TICKER_NULL_SLOT, event_stop,
			(void *)STATE_ABORT, ticker_success_assert,
			(void *)__LINE__);
		LL_ASSERT((ticker_status == TICKER_STATUS_SUCCESS) ||
			  (ticker_status == TICKER_STATUS_BUSY));
	}
...
#undef RADIO_DEFERRED_PREEMPT
	...
	if (_radio.ticks_active_to_start != _ticks_active_to_start) {
		...
		*ticks_active_to_start = _radio.ticks_active_to_start;
		...
		ticks_to_start_new =
			MAX(_radio.ticks_active_to_start, *ticks_xtal_to_start);
	...
	{
		static memq_link_t s_link;
		static struct mayfly s_mfy_xtal_stop_calc = {
			0, 0, &s_link, NULL, mayfly_xtal_stop_calc
		};
		u32_t retval;
...
again:
	...
	if (util_ones_count_get((u8_t *)&adv_aa_check, sizeof(adv_aa_check)) <=
	    1) {
		goto again;
	}
...
void radio_event_adv_prepare(u32_t ticks_at_expire, u32_t remainder, u16_t lazy,
			     void *context)
{
	ARG_UNUSED(lazy);
	ARG_UNUSED(context);
...
static void adv_setup(void)
{
	...
	if (_radio.advertiser.scan_data.first !=
	    _radio.advertiser.scan_data.last) {
		u8_t first;
	...
	pdu = (void *)_radio.advertiser.adv_data
		      .data[_radio.advertiser.adv_data.first];
#if defined(CONFIG_BT_CTLR_PRIVACY)
	if (upd) {
		struct pdu_adv *scan_pdu =
			(void *)_radio.advertiser.scan_data
				.data[_radio.advertiser.scan_data.first];
...
static inline void ticker_stop_adv_stop_active(void)
{
	...
	if (ret_cb_evt == TICKER_STATUS_SUCCESS) {
		...
		ret = ticker_stop(RADIO_TICKER_INSTANCE_ID_RADIO,
				  RADIO_TICKER_USER_ID_JOB,
				  RADIO_TICKER_ID_MARKER_0, ticker_if_done,
				  (void *)&ret_cb_m0);
...
static void ticker_stop_adv_stop(u32_t status, void *params)
{
	...
	/* Generate an event in WORKER Prio */
	retval =
		mayfly_enqueue(RADIO_TICKER_USER_ID_JOB,
			       RADIO_TICKER_USER_ID_WORKER, 0, &s_mfy_adv_stop);
	LL_ASSERT(!retval);
#endif /* CONFIG_BT_PERIPHERAL */
}
			       ...
			       u16_t lazy, void *context)
{
	...
	if (_radio.scanner.conn) {
		...
		retval =
			mayfly_enqueue(RADIO_TICKER_USER_ID_WORKER,
				       RADIO_TICKER_USER_ID_JOB, 1,
				       ...
				       void (*fp_mayfly_select_or_use)(void *))
{
	...
	/* set instant */
	conn->llcp.conn_upd.instant = event_counter + conn->latency + 6;
...
#if defined(CONFIG_BT_CTLR_SCHED_ADVANCED)
	{
		...
		conn->llcp.conn_upd.pdu_win_offset =
			(u16_t *)&pdu_ctrl_tx->llctrl.conn_update_ind.win_offset;
					...
					u32_t ticks_at_expire)
{
	...
	instant_latency =
		(event_counter - conn->llcp.conn_upd.instant) & 0xffff;
	if (conn->llcp_cu.state != LLCP_CUI_STATE_INPROG) {
#if defined(CONFIG_BT_CTLR_SCHED_ADVANCED)
		static memq_link_t s_link;
		static struct mayfly s_mfy_sched_offset = { 0, 0, &s_link, 0,
							    0 };
		...
		conn->supervision_reload = RADIO_CONN_EVENTS(
			(conn->llcp_cu.timeout * 10 * 1000), conn_interval_us);
		conn->procedure_reload =
			RADIO_CONN_EVENTS((40 * 1000 * 1000), conn_interval_us);
		...
		ticker_status = ticker_stop(RADIO_TICKER_INSTANCE_ID_RADIO,
					    RADIO_TICKER_USER_ID_WORKER,
					    ticker_id, ticker_stop_conn_assert,
					    (void *)(u32_t)ticker_id);
		LL_ASSERT((ticker_status == TICKER_STATUS_SUCCESS) ||
			  (ticker_status == TICKER_STATUS_BUSY));
		ticker_status = ticker_start(
			RADIO_TICKER_INSTANCE_ID_RADIO,
			RADIO_TICKER_USER_ID_WORKER, ticker_id, ticks_at_expire,
			ticks_win_offset, HAL_TICKER_US_TO_TICKS(periodic_us),
			HAL_TICKER_REMAINDER(periodic_us), TICKER_NULL_LAZY,
			(ticks_slot_offset + conn->hdr.ticks_slot),
#if defined(CONFIG_BT_PERIPHERAL) && defined(CONFIG_BT_CENTRAL)
			conn->role ? event_slave_prepare : event_master_prepare,
#elif defined(CONFIG_BT_PERIPHERAL)
			event_slave_prepare,
#else
			event_master_prepare,
#endif
			conn, ticker_start_conn_assert, (void *)__LINE__);
		LL_ASSERT((ticker_status == TICKER_STATUS_SUCCESS) ||
			  (ticker_status == TICKER_STATUS_BUSY));
				     ...
				     u16_t event_counter)
{
	if (conn->llcp.chan_map.initiate) {
		...
		memcpy(&conn->data_chan_map[0], &conn->llcp.chan_map.chm[0],
		       sizeof(conn->data_chan_map));
		conn->data_chan_count = util_ones_count_get(
			&conn->data_chan_map[0], sizeof(conn->data_chan_map));
		conn->chm_update = 1U;
...
static inline void event_fex_prep(struct connection *conn)
{
	...
	if (node_tx) {
		...
		pdu_ctrl_tx->len =
			...
			sizeof(struct pdu_data_llctrl_feature_req);
		pdu_ctrl_tx->llctrl.opcode =
			!conn->role ? PDU_DATA_LLCTRL_TYPE_FEATURE_REQ :
				      PDU_DATA_LLCTRL_TYPE_SLAVE_FEATURE_REQ;
		(void)memset(&pdu_ctrl_tx->llctrl.feature_req.features[0], 0x00,
...
static inline void event_vex_prep(struct connection *conn)
{
	...
	if (conn->llcp_version.tx == 0) {
		...
		pdu_ctrl_rx->len =
			offsetof(struct pdu_data_llctrl, version_ind) +
			sizeof(struct pdu_data_llctrl_version_ind);
		pdu_ctrl_rx->llctrl.opcode = PDU_DATA_LLCTRL_TYPE_VERSION_IND;
					...
					u32_t ticks_at_expire)
{
	...
	{
		static memq_link_t s_link;
		static struct mayfly s_mfy_sched_offset = {
			0, 0, &s_link, NULL, mayfly_sched_free_win_offset_calc
		};
		u32_t retval;
...
static inline int event_len_prep(struct connection *conn)
{
	...
	case LLCP_LENGTH_STATE_RESIZE_RSP: {
		...
		if (((free_count_conn + 1) == _radio.connection_count) ||
			...
			if (conn->max_rx_octets <
			    (PDU_AC_SIZE_MAX + PDU_AC_SIZE_EXTRA)) {
				_radio.packet_rx_data_size = MROUND(
					offsetof(struct radio_pdu_node_rx,
						 pdu_data) +
					(PDU_AC_SIZE_MAX + PDU_AC_SIZE_EXTRA));
			} else {
...
static inline void event_phy_req_prep(struct connection *conn)
{
	...
	case LLCP_PHY_STATE_REQ: {
		...
		pdu_ctrl_tx = (void *)node_tx->pdu_data;
		pdu_ctrl_tx->ll_id = PDU_DATA_LLID_CTRL;
		pdu_ctrl_tx->len = offsetof(struct pdu_data_llctrl, phy_req) +
				   sizeof(struct pdu_data_llctrl_phy_req);
	...
	if (conn->llcp.phy_upd_ind.initiate) {
		...
		if (conn->llcp.phy_upd_ind.tx) {
			...
			if (tx_time >=
			    RADIO_PKT_TIME(PDU_DC_PAYLOAD_SIZE_MIN, 0)) {
				eff_tx_time =
					MIN(tx_time, conn->default_tx_time);
#if defined(CONFIG_BT_CTLR_PHY_CODED)
				eff_tx_time = MAX(
					eff_tx_time,
					RADIO_PKT_TIME(PDU_DC_PAYLOAD_SIZE_MIN,
				...
				eff_tx_time = RADIO_PKT_TIME(
					PDU_DC_PAYLOAD_SIZE_MIN, 0);
			}
#endif /* CONFIG_BT_CTLR_DATA_LENGTH */
		}
...
static void event_connection_prepare(u32_t ticks_at_expire, u32_t remainder,
				     u16_t lazy, struct connection *conn)
{
	u16_t event_counter;
			...
			void *context)
{
	...
	rx_packet_set(
		conn,
		(void *)_radio.packet_rx[_radio.packet_rx_last]->pdu_data);

	/* Setup Radio Channel */
	if (conn->data_chan_sel) {
#if defined(CONFIG_BT_CTLR_CHAN_SEL_2)
		data_chan_use =
			chan_sel_2(conn->event_counter - 1, conn->data_chan_id,
				   &conn->data_chan_map[0],
		...
		data_chan_use =
			chan_sel_1(&conn->data_chan_use, conn->data_chan_hop,
				   conn->latency_event, &conn->data_chan_map[0],
				   conn->data_chan_count);
	}
	...
	/* current window size */
	conn->slave.window_size_event_us += conn->slave.window_size_prepare_us;
...
static void rx_packet_set(struct connection *conn, struct pdu_data *pdu_data_rx)
{
	...
	if (conn->enc_rx) {
		...
		radio_pkt_rx_set(
			radio_ccm_rx_pkt_set(&conn->ccm_rx, phy, pdu_data_rx));
	} else {
...
static inline u8_t packet_rx_acquired_count_get(void)
{
	if (_radio.packet_rx_acquire >= _radio.packet_rx_last) {
		return (_radio.packet_rx_acquire - _radio.packet_rx_last);
	} else {
		return (_radio.packet_rx_count - _radio.packet_rx_last +
			_radio.packet_rx_acquire);
	}
}
...
static void pdu_node_tx_release(u16_t handle, struct radio_pdu_node_tx *node_tx)
{
	u8_t last;
...
static void tx_packet_set(struct connection *conn, struct pdu_data *pdu_data_tx)
{
	...
	if (conn->enc_tx) {
		...
		radio_pkt_tx_set(
			radio_ccm_tx_pkt_set(&conn->ccm_tx, pdu_data_tx));
	} else {
				...
				struct pdu_data **pdu_data_tx)
{
	...
	if (/* empty packet */
		...
		_pdu_data_tx->len =
			conn->packet_tx_head_len - conn->packet_tx_head_offset;
		_pdu_data_tx->md = 0U;
...
static u8_t chan_map_update(struct connection *conn,
			    struct pdu_data *pdu_data_rx)
{
	if (((pdu_data_rx->llctrl.chan_map_ind.instant - conn->event_counter) &
	     0xffff) > 0x7fff) {
		return BT_HCI_ERR_INSTANT_PASSED;
	}
	...
	memcpy(&conn->llcp.chan_map.chm[0],
	       ...
	       sizeof(conn->llcp.chan_map.chm));
	conn->llcp.chan_map.instant = pdu_data_rx->llctrl.chan_map_ind.instant;
	conn->llcp.chan_map.initiate = 0U;
...
static u8_t enc_rsp_send(struct connection *conn)
{
	...
	memcpy(&conn->ccm_rx.iv[4], &pdu_ctrl_tx->llctrl.enc_rsp.ivs[0], 4);
...
static u8_t phy_rsp_send(struct connection *conn, struct pdu_data *pdu_data_rx)
{
	...
	if (conn->llcp_phy.ack == conn->llcp_phy.req) {
		...
		conn->llcp_phy.tx = conn->phy_pref_tx;
		conn->llcp_phy.rx = conn->phy_pref_rx;
...
static u8_t role_disable(u8_t ticker_id_primary, u8_t ticker_id_stop)
{
	...
	case RADIO_TICKER_ID_ADV:
		ticks_xtal_to_start = _radio.advertiser.hdr.ticks_xtal_to_start;
	...
	case RADIO_TICKER_ID_SCAN:
		ticks_xtal_to_start = _radio.scanner.hdr.ticks_xtal_to_start;
		ticks_active_to_start =
			_radio.scanner.hdr.ticks_active_to_start;
		break;
	default:
		if (ticker_id_primary >= RADIO_TICKER_ID_FIRST_CONNECTION) {
			...
			ticks_xtal_to_start = conn->hdr.ticks_xtal_to_start;
			ticks_active_to_start = conn->hdr.ticks_active_to_start;
		} else {
			LL_ASSERT(0);
		}
		break;
	...
	if ((_radio.ticker_id_prepare == ticker_id_primary) ||
		...
		role_active_disable(ticker_id_stop, ticks_xtal_to_start,
				    ticks_active_to_start);
	}
...
#endif /* !CONFIG_BT_CTLR_ADV_EXT */
{
	...
	pdu_adv = (void *)&_radio.advertiser.adv_data
			  .data[_radio.advertiser.adv_data.last][0];
	...
	if (pdu_adv->type == PDU_ADV_TYPE_DIRECT_IND) {
		...
		slot_us = (RADIO_TICKER_START_PART_US + 376 + 152 + 176 + 152 +
			   376) *
			  chan_cnt;
	}
	...
	ticks_slot_offset = MAX(_radio.advertiser.hdr.ticks_active_to_start,
				_radio.advertiser.hdr.ticks_xtal_to_start);
	...
	_radio.advertiser.is_hdcd =
		!interval && (pdu_adv->type == PDU_ADV_TYPE_DIRECT_IND);
	if (_radio.advertiser.is_hdcd) {
		ret = ticker_start(
			RADIO_TICKER_INSTANCE_ID_RADIO,
			RADIO_TICKER_USER_ID_APP, RADIO_TICKER_ID_ADV,
			ticks_anchor, 0,
			(ticks_slot_offset + _radio.advertiser.hdr.ticks_slot),
			TICKER_NULL_REMAINDER, TICKER_NULL_LAZY,
			(ticks_slot_offset + _radio.advertiser.hdr.ticks_slot),
			radio_event_adv_prepare, NULL, ticker_if_done,
			(void *)&ret_cb);
		...
		ret = ticker_start(
			RADIO_TICKER_INSTANCE_ID_RADIO,
			RADIO_TICKER_USER_ID_APP, RADIO_TICKER_ID_ADV_STOP,
			ticks_anchor,
			HAL_TICKER_US_TO_TICKS((1280 * 1000) +
					       RADIO_TICKER_XTAL_OFFSET_US),
			TICKER_NULL_PERIOD, TICKER_NULL_REMAINDER,
			TICKER_NULL_LAZY, TICKER_NULL_SLOT, event_adv_stop,
			NULL, ticker_if_done, (void *)&ret_cb);
	} else
#endif /* CONFIG_BT_PERIPHERAL */
	{
		ret = ticker_start(
			...
			RADIO_TICKER_USER_ID_APP, RADIO_TICKER_ID_ADV,
			ticks_anchor, 0,
			HAL_TICKER_US_TO_TICKS((u64_t)interval * 625),
			TICKER_NULL_REMAINDER, TICKER_NULL_LAZY,
			(ticks_slot_offset + _radio.advertiser.hdr.ticks_slot),
			radio_event_adv_prepare, NULL, ticker_if_done,
			(void *)&ret_cb);
...
u8_t radio_adv_disable(void)
{
	...
	status = role_disable(RADIO_TICKER_ID_ADV, RADIO_TICKER_ID_ADV_STOP);
	if (!status) {
		struct connection *conn;
			...
			u8_t rpa_gen, u8_t rl_idx)
{
	...
	_radio.scanner.ticks_window =
		HAL_TICKER_US_TO_TICKS((u64_t)window * 625);
	_radio.scanner.filter_policy = filter_policy;
...
u8_t radio_scan_disable(bool scanner)
{
	...
	status = role_disable(RADIO_TICKER_ID_SCAN, RADIO_TICKER_ID_SCAN_STOP);
			   ...
			   u16_t latency, u16_t timeout)
{
	...
	conn_interval_us = (u32_t)_radio.scanner.conn_interval * 1250;
	conn->supervision_reload = RADIO_CONN_EVENTS(
		(_radio.scanner.conn_timeout * 10 * 1000), conn_interval_us);
	conn->supervision_expire = 0U;
	conn->procedure_reload =
		RADIO_CONN_EVENTS((40 * 1000 * 1000), conn_interval_us);
	conn->procedure_expire = 0U;
...
u8_t ll_chm_update(u8_t *chm)
{
	...
	memcpy(&_radio.data_chan_map[0], chm, sizeof(_radio.data_chan_map));
	_radio.data_chan_count = util_ones_count_get(
		&_radio.data_chan_map[0], sizeof(_radio.data_chan_map));
...
u8_t ll_enc_req_send(u16_t handle, u8_t *rand, u8_t *ediv, u8_t *ltk)
{
	...
	if (node_tx) {
		...
		if (!conn->enc_rx && !conn->enc_tx) {
			...
			pdu_data_tx->llctrl.opcode =
				PDU_DATA_LLCTRL_TYPE_ENC_REQ;
			enc_req = (void *)&pdu_data_tx->llctrl.enc_req;
...
u8_t ll_apto_set(u16_t handle, u16_t apto)
{
	...
	conn->apto_reload =
		RADIO_CONN_EVENTS(apto * 10 * 1000, conn->conn_interval * 1250);
...
u8_t ll_tx_pwr_lvl_get(u8_t handle_type, u16_t handle, u8_t type,
		       s8_t *tx_pwr_lvl)
{
	switch (handle_type) {
...
u8_t ll_tx_pwr_lvl_set(u8_t handle_type, u16_t handle, s8_t *tx_pwr_lvl)
{
#if defined(CONFIG_BT_CTLR_TX_PWR_DYNAMIC_CONTROL)
	...
	*handle = _radio.pkt_release[_first].handle;
	do {
		...
		if ((!node_tx) || (node_tx == (void *)1) ||
		    ((((u32_t)node_tx & ~(0x00000003)) != 0) && (pdu_data_tx) &&
		     (pdu_data_tx->len != 0) &&
	...
	if (_radio.link_rx_head != _radio.link_rx_tail) {
		...
		if (!cmplt) {
			...
			do {
				...
				cmplt_curr = tx_cmplt_get(
					&h, &first, _radio.packet_release_last);
			} while ((cmplt_prev != 0) ||
				 (cmplt_prev != cmplt_curr));
...
void ll_rx_dequeue(void)
{
	...
	if (0) {
#if defined(CONFIG_BT_CONN)
	} else if (node_rx->hdr.type == NODE_RX_TYPE_CONNECTION) {
		struct radio_le_conn_cmplt *radio_le_conn_cmplt;
		...
		if (conn) {
			struct radio_pdu_node_rx *node_rx =
				(void *)&conn->llcp_terminate.radio_pdu_node_rx;

			mem_release(node_rx->hdr.link, &_radio.link_rx_free);
			mem_release(conn, &_radio.conn_free);
		}
...
void ll_rx_mem_release(void **node_rx)
{
	...
	while (_node_rx) {
		...
		case NODE_RX_TYPE_NONE:
			...
			mem_release(_node_rx_free, &_radio.pkt_rx_data_free);
			break;
...
int ll_tx_mem_enqueue(u16_t handle, void *node_tx)
{
	...
	LL_ASSERT(pdu_data->len <=
		  (_radio.packet_tx_data_size -
		   offsetof(struct radio_pdu_node_tx, pdu_data) -
		   offsetof(struct pdu_data, lldata)));
	...
	if (conn->role && conn->latency_event && !conn->slave.latency_cancel) {
		...
		ticker_status = ticker_update(
			RADIO_TICKER_INSTANCE_ID_RADIO,
			RADIO_TICKER_USER_ID_APP,
			RADIO_TICKER_ID_FIRST_CONNECTION + conn->handle, 0, 0,
			0, 0, 1, 0, ticker_op_latency_cancelled, (void *)conn);
		LL_ASSERT((ticker_status == TICKER_STATUS_SUCCESS) ||
			  (ticker_status == TICKER_STATUS_BUSY));
	}
