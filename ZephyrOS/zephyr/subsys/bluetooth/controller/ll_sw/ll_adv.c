...
u8_t ll_adv_params_set(u8_t handle, u16_t evt_prop, u32_t interval,
		       u8_t adv_type, u8_t own_addr_type, u8_t direct_addr_type,
		       u8_t const *const direct_addr, u8_t chan_map,
		       u8_t filter_policy, u8_t *tx_pwr, u8_t phy_p, u8_t skip,
		       u8_t phy_s, u8_t sid, u8_t sreq)
{
	u8_t const pdu_adv_type[] = {
...
u8_t ll_adv_params_set(u16_t interval, u8_t adv_type, u8_t own_addr_type,
		       ...
		       u8_t chan_map, u8_t filter_policy)
{
	...
	if (pdu->type == PDU_ADV_TYPE_DIRECT_IND) {
		...
		*(u8_t *)h = 0;
		...
		if (!p->adv_mode && (!_h.aux_ptr || (!(evt_prop & BIT(5)) &&
						     (phy_p != BIT(2))))) {
		...
		/* Tx Power flag */
		if (evt_prop & BIT(6) && (!_h.aux_ptr || (phy_p != BIT(2)))) {
			h->tx_pwr = 1;
			ptr++;
		}
...
{
	// IFW below
	// IFW_DEBUG_LOG("LL scan response set here.");
	// IFW above
	...
	prev = (struct pdu_adv *)&radio_scan_data
		       ->data[radio_scan_data->last][0];
...
u8_t ll_adv_enable(u16_t handle, u8_t enable, u8_t at_anchor,
		   u32_t ticks_anchor, u8_t retry, u8_t scan_window,
		   u8_t scan_delay)
...
#endif /* !CONFIG_BT_CTLR_ADV_EXT || !CONFIG_BT_HCI_MESH_EXT */
{
	...
	radio_adv_data = radio_adv_data_get();
	pdu_adv =
		(struct pdu_adv *)&radio_adv_data->data[radio_adv_data->last][0];

	radio_scan_data = radio_scan_data_get();
	pdu_scan = (struct pdu_adv *)&radio_scan_data
			   ->data[radio_scan_data->last][0];
	...
	if (0) {
		...
		if (ll_adv.own_addr_type == BT_ADDR_LE_PUBLIC_ID ||
			...
			rl_idx = ll_rl_find(ll_adv.id_addr_type, ll_adv.id_addr,
					    NULL);
	...
	status = radio_adv_enable(ll_adv.phy_p, ll_adv.interval,
				  ll_adv.chan_map, ll_adv.filter_policy, rl_idx,
#else /* !CONFIG_BT_CTLR_ADV_EXT */
	status = radio_adv_enable(ll_adv.interval, ll_adv.chan_map,
				  ll_adv.filter_policy, rl_idx,
#endif /* !CONFIG_BT_CTLR_ADV_EXT */
				  at_anchor, ticks_anchor, retry, scan_window,
				  scan_delay);
#else /* !CONFIG_BT_HCI_MESH_EXT */
#if defined(CONFIG_BT_CTLR_ADV_EXT)
	status =
		radio_adv_enable(ll_adv.phy_p, ll_adv.interval, ll_adv.chan_map,
				 ll_adv.filter_policy, rl_idx);
#else /* !CONFIG_BT_CTLR_ADV_EXT */
	status = radio_adv_enable(ll_adv.interval, ll_adv.chan_map,
				  ll_adv.filter_policy, rl_idx);
