...
static inline void spi_dump_message(const u8_t *pre, u8_t *buf, u8_t size)
{
	u8_t i, c;
...
static inline int bt_spi_transceive(void *tx, u32_t tx_len, void *rx,
				    u32_t rx_len)
{
	spi_tx_buf.buf = tx;
...
static void bt_spi_rx_thread(void)
{
	...
	while (true) {
		...
		do {
			...
			// if (IFW_HOOK_CHECK(IFW_SYS_BLE_HCI_SPI, IFW_DIR_RX, rxmsg)) {
			// 	BT_ERR("IFW Drop BT buf type %d", rxmsg[0]);
			// 	continue;
			// }
			// #endif
...
static int bt_spi_send(struct net_buf *buf)
{
	...
	// #ifdef CONFIG_USE_PERIPHERAL_FILTER
	// 	if (IFW_HOOK_CHECK(IFW_SYS_BLE_HCI_SPI, IFW_DIR_TX, buf)) {
	// 		BT_ERR("IFW Drop spi buf in bt_spi_send!");
	// 		goto out;
	// 	}
	// #endif
...
static int bt_spi_open(void)
{
	...
	k_thread_create(&spi_rx_thread_data, spi_rx_stack,
			...
			K_PRIO_COOP(CONFIG_BT_RX_PRIO - 1), 0, K_NO_WAIT);
